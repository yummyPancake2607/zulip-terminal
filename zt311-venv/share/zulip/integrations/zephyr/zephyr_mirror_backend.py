...
def to_zephyr_username(zulip_username: str) -> str:
    ...
    if not match_user:
        raise Exception(
            f"Could not parse Zephyr realm for cross-realm user {zulip_username}"
        )
    ...
    if os.stat(
        ...
        while backoff.keep_going():
            try:
                ...
                logger.exception(
                    "Error restarting mirroring script; trying again... Traceback:"
                )
                backoff.fail()
...
def parse_crypt_table(zephyr_class: str, instance: str) -> Optional[str]:
    ...
    for line in crypt_table.readlines():
        if line.strip() == "":
            ...
            continue
        match = re.match(
            r"^crypt-(?P<class>\S+):\s+((?P<algorithm>(AES|DES)):\s+)?(?P<keypath>\S+)$",
            line,
        )
        if match is None:
...
def process_notice(notice: "zephyr.ZNotice", log: Optional[IO[str]]) -> None:
    ...
    if (zephyr_class not in current_zephyr_subs) and not is_personal:
        ...
        return
    if notice.format.startswith("Zephyr error: See") or notice.format.endswith(
        "@(@color(blue))"
    ):
    ...
    logger.info(
        f"Received a message on {zephyr_class}/{notice.instance} from {notice.sender}..."
    )
    if log is not None:
...
def zephyr_init_autoretry() -> None:
    ...
    while backoff.keep_going():
        try:
            ...
            logger.exception(
                "Error initializing Zephyr library (retrying).  Traceback:"
            )
            backoff.fail()
...
def zephyr_load_session_autoretry(session_path: str) -> None:
    ...
    while backoff.keep_going():
        try:
            ...
            logger.exception(
                "Error loading saved Zephyr session (retrying).  Traceback:"
            )
            backoff.fail()
...
def send_zephyr(zwrite_args: List[str], content: str) -> Tuple[int, str]:
    ...
    if p.returncode:
        ...
        logger.warning(
            "zwrite command '{}' printed the following warning:".format(
                " ".join(zwrite_args)
            )
        )
    if stderr:
...
def zcrypt_encrypt_content(
    zephyr_class: str, instance: str, content: str
) -> Optional[str]:
    keypath = parse_crypt_table(zephyr_class, instance)
    if keypath is None:
        return None
...
Feedback button or at support@zulip.com."""
    ...
    if message["type"] == "stream":
        ...
        if match_whitespace_instance:
            ...
            instance == f"instance {zephyr_class}"
            or instance
...
def maybe_forward_to_zephyr(message: Dict[str, Any]) -> None:
    ...
    if message["sender_email"] == zulip_account_email:
        ...
        if float(message["timestamp"]) < timestamp_now - 15:
            logger.warning(
                "Skipping out of order message: {} < {}".format(
    ...
    if len(zephyr_subscriptions) != 0:
        ...
        if verbose:
            if already is not None and len(already) > 0:
                logger.info(
                    "\nAlready subscribed to: {}".format(
                        ", ".join(list(already.values())[0])
                    )
                )
            if new is not None and len(new) > 0:
                logger.info(
                    "\nSuccessfully subscribed to: {}".format(
                        ", ".join(list(new.values())[0])
                    )
                )
    ...
    for line in open(subs_file).readlines():
        ...
        try:
            ...
            if not valid_stream_name(cls):
                if verbose:
                    logger.error(
                        f"Skipping subscription to unsupported class name: [{line}]"
                    )
                continue
        except Exception:
            if verbose:
...
def open_logger() -> logging.Logger:
    if options.log_path is not None:
        ...
        f = tempfile.NamedTemporaryFile(
            prefix=f"zulip-log.{options.user}.", delete=False
        )
        log_file = f.name
...
def parse_args() -> Tuple[optparse.Values, List[str]]:
    ...
    parser.add_option(
        "--noshard", default=False, help=optparse.SUPPRESS_HELP, action="store_true"
    )
    parser.add_option(
        "--resend-log", dest="logs_to_resend", help=optparse.SUPPRESS_HELP
    )
    parser.add_option(
        "--enable-resend-log", dest="resend_log_path", help=optparse.SUPPRESS_HELP
    )
    ...
    parser.add_option(
        "--verbose", default=False, help=optparse.SUPPRESS_HELP, action="store_true"
    )
