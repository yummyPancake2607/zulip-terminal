...
def matrix_login(matrix_client: Any, matrix_config: Dict[str, Any]) -> None:
    try:
        matrix_client.login_with_password(
            matrix_config["mxid"], matrix_config["password"]
        )
    except MatrixRequestError as exception:
        if exception.code == 403:
            raise Bridge_FatalMatrixException("Bad mxid or password.")
        else:
            raise Bridge_FatalMatrixException(
                "Check if your server details are correct."
            )
...
def get_message_content_from_event(
    event: Dict[str, Any], no_noise: bool
) -> Optional[str]:
    ...
    if event["type"] == "m.room.member":
        ...
        if (
            ...
            or event["content"]["msgtype"] == "m.emote"
        ):
...
def read_configuration(config_file: str) -> Dict[str, Dict[str, str]]:
    ...
    if set(config.sections()) != {"matrix", "zulip"}:
        raise Bridge_ConfigException(
            "Please ensure the configuration has zulip & matrix sections."
        )
...
def write_sample_config(target_path: str, zuliprc: Optional[str]) -> None:
    if os.path.exists(target_path):
        raise Bridge_ConfigException(
            f"Path '{target_path}' exists; not overwriting existing file."
        )
...
def main() -> None:
    ...
    while backoff.keep_going():
        ...
        try:
            ...
            room.add_listener(
                matrix_to_zulip(
                    zulip_client, zulip_config, matrix_config, options.no_noise
                )
            )
