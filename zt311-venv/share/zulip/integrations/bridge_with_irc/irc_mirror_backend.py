    ...
    def on_welcome(self, c: ServerConnection, e: Event) -> None:
        ...
        def forward_to_irc(msg: Dict[str, Any]) -> None:
            ...
            if is_a_stream:
                ...
                at_the_specified_subject = (
                    msg["subject"].casefold() == self.topic.casefold()
                )
                if in_the_specified_stream and at_the_specified_subject:
                    msg["content"] = (
                        "@**{}**: ".format(msg["sender_full_name"])
                    ) + msg["content"]
        ...
        z2i = mp.Process(
            target=self.zulip_client.call_on_each_message, args=(forward_to_irc,)
        )
        z2i.start()
