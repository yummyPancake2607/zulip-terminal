...
def _is_valid_key(char):
    return urwid.is_wide_char(char, 0) or (len(char) == 1 and ord(char) >= 32)
...
class ReadlineEdit(urwid.Edit):
    ...
    def paste(self):
        ...
        self.set_edit_text(
            self.edit_text[: self.edit_pos] + text + self.edit_text[self.edit_pos :]
        )
    ...
    def forward_word(self):
        for match in self._word_regex2.finditer(self._edit_text[self._edit_pos :]):
            self.set_edit_pos(self._edit_pos + match.end(1))
            return
    ...
    def backward_kill_line(self):
        for pos in reversed(range(0, self.edit_pos)):
            if self.edit_text[pos] == "\n":
                ...
                self.set_edit_text(
                    self._edit_text[: pos + 1] + self._edit_text[self.edit_pos :]
                )
                self.edit_pos = pos + 1
                return
    ...
    def backward_kill_word(self):
        ...
        self.set_edit_text(self._edit_text[: self._edit_pos] + self._edit_text[pos:])

    def kill_word(self):
        pos = self._edit_pos
        self.forward_word()
        self._paste_buffer.append(self.edit_text[pos : self.edit_pos])
        self.set_edit_text(self._edit_text[0:pos] + self._edit_text[self._edit_pos :])
    ...
    def end_of_line(self):
        ...
        if self.edit_pos < text_length and self.edit_text[self.edit_pos] == "\n":
            self.forward_char()
