...
class Screen:
    ...
    def start(self):
        ...
        urwid_id = "%09d%09d" % (random.randrange(10**9), random.randrange(10**9))
        ...
        self.input_fd = os.open(self.pipe_name + ".in", os.O_NONBLOCK | os.O_RDONLY)
        self.input_tail = ""
        self.content_head = (
            "Content-type: "
...
def handle_short_request():
    ...
    if os.environ.get("HTTP_X_URWID_METHOD", None) == "polling":
        ...
        s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        try:
            s.connect(os.path.join(_prefs.pipe_dir, "urwid" + urwid_id + ".update"))
    ...
    try:
        fd = os.open(
            (os.path.join(_prefs.pipe_dir, "urwid" + urwid_id + ".in")), os.O_WRONLY
        )
    except OSError:
        sys.stdout.write("Status: 404 Not Found\r\n\r\n")
        return True
