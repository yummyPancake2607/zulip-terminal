...
_COLOR_VALUES_256 = (
    ...
    + [
        (r, g, b)
        for r in _CUBE_STEPS_256
        for g in _CUBE_STEPS_256
...
_COLOR_VALUES_88 = (
    ...
    + [
        (r, g, b)
        for r in _CUBE_STEPS_88
        for g in _CUBE_STEPS_88
...
def _value_lookup_table(values, size):
    ...
    middle_values = (
        [0]
        + [(values[i] + values[i + 1] + 1) // 2 for i in range(len(values) - 1)]
        + [size]
    )
    lookup_table = []
...
def _color_desc_256(num):
    ...
    if num < _GRAY_START_256:
        ...
        return "#%x%x%x" % (
            _CUBE_STEPS_256_16[r],
            _CUBE_STEPS_256_16[g],
...
def _true_to_256(desc):
    ...
    c256 = _parse_color_256(
        "#"
        + "".join(
            [format(int(x, 16) // 16, "x") for x in [desc[1:3], desc[3:5], desc[5:7]]]
        )
    )
...
class AttrSpec(object):
    def __init__(self, fg, bg, colors=256):
        ...
        self._value = (
            0 | _HIGH_88_COLOR * (colors == 88) | _HIGH_TRUE_COLOR * (colors == 2**24)
        )
        self.foreground = fg
        self.background = bg
        if self.colors > colors:
            raise AttrSpecError(
                (
                    "foreground/background (%s/%s) require "
    ...
    background_number = property(lambda s: (s._value & _BG_COLOR_MASK) >> _BG_SHIFT)
        ...
        for part in foreground.split(","):
            ...
            if part in _ATTRIBUTES:
                ...
                if flags & _ATTRIBUTES[part]:
                    raise AttrSpecError(
                        ("Setting %s specified more than" + "once in foreground (%s)")
                        % (repr(part), repr(foreground))
                    )
            ...
            if scolor is None:
                raise AttrSpecError(
                    ("Unrecognised color specification %s " + "in foreground (%s)")
                    % (repr(part), repr(foreground))
                )
            if color is not None:
                raise AttrSpecError(
                    ("More than one color given for " + "foreground (%s)")
                    % (repr(foreground),)
                )
            color = scolor
        if color is None:
    ...
    def _set_background(self, background):
        ...
        if color is None:
            raise AttrSpecError(
                ("Unrecognised color specification " + "in background (%s)")
                % (repr(background),)
            )
...
class BaseScreen(with_metaclass(signals.MetaSignals, object)):
    ...
    def register_palette_entry(
        ...
        signals.emit_signal(
            self, UPDATE_PALETTE_ENTRY, name, basic, mono, high_88, high_256, high_true
        )
