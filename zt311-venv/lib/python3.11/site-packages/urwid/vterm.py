...
class TermCanvas(Canvas):
        ...
        if height > self.height:
            ...
            for y in xrange(self.height, height):
                ...
                if len(last_line) < self.width:
                    last_line += [self.empty_char()] * (self.width - len(last_line))
                else:
    ...
    def parse_escape(self, char):
        if self.parsestate == 1:
            ...
            self.parsestate == 2
            and self.escbuf.startswith(B("P"))
            and len(self.escbuf) == 8
        ):
    ...
    def move_cursor(self, x, y, relative_x=False, relative_y=False, relative=False):
        """
    ...
    def content(self, trim_left=0, trim_right=0, cols=None, rows=None, attr_map=None):
...
class Terminal(Widget):
    ...
    def __init__(
        self, command, env=None, main_loop=None, escape_sequence=None, encoding="utf-8"
    ):
        """
