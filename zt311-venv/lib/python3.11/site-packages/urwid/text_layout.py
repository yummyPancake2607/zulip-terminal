    ...
    def layout(self, text, width, align, wrap):
        ...
        * (column width of insert text, offset, "insert text")
        ...
        raise NotImplementedError(
            "This function must be overridden by a real"
        ...
        if wrap in ("clip", "ellipsis"):
            ...
            while p <= len(text):
                ...
                if wrap == "ellipsis" and sc > width:
                    trimmed = True
                    spos, n_end, pad_left, pad_right = calc_trim_text(
                        text, p, n_cr, 0, width - 1
                    )
        ...
        if self.text:
            ...
            return [
                (
                    ...
                    self.offs,
                    bytes().ljust(pad_left)
                    + self.text[spos:epos]
                    + bytes().ljust(pad_right),
                )
            ]
        elif self.end:
    ...
    for seg in line_layout:
        ...
        if s.offs is not None:
            if s.end is not None:
                if current_sc <= pref_col and pref_col < current_sc + s.sc:
            ...
            if closest_sc is None or (
                abs(pref_col - current_sc) < abs(pref_col - closest_sc)
            ):
