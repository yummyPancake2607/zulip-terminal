...
class GridFlow(WidgetWrap, WidgetContainerMixin, WidgetContainerListContentsMixin):
    ...
    def _set_cells(self, widgets):
        ...
        self.contents = [(new, (GIVEN, self._cell_width)) for new in widgets]
    ...
    def _set_cell_width(self, width):
        focus_position = self.focus_position
        self.contents = [(w, (GIVEN, width)) for (w, options) in self.contents]
        self.focus_position = focus_position
    ...
    focus = property(
        get_focus, doc="the child widget in focus or None when GridFlow is empty"
    )
...
class Overlay(Widget, WidgetContainerMixin, WidgetContainerListContentsMixin):
    ...
    def keypress(self, size, key):
        ...
        return self.top_w.keypress(
            self.top_w_size(size, *self.calculate_padding_filler(size, True)), key
        )
    ...
    focus = property(
        _get_focus, doc="the top widget in this overlay is always in focus"
    )
    ...
    def _set_focus_position(self, position):
        ...
        if position != 1:
            raise IndexError(
                "Overlay widget focus_position currently "
    ...
    def _contents__getitem__(self, index):
        ...
        raise IndexError("Overlay.contents has no position %r" % (index,))
    ...
    def _contents__setitem__(self, index, value):
        ...
        if index == 0:
            ...
            try:
                ...
                raise OverlayError("top_options is invalid: %r" % (value_options,))
...
class Frame(Widget, WidgetContainerMixin):
    ...
    def _get_focus(self):
        return {"header": self._header, "footer": self._footer, "body": self._body}[
            self.focus_part
        ]
    ...
    def _contents__delitem__(self, key):
        ...
        if (key == "header" and self._header is None) or (
            key == "footer" and self._footer is None
        ):
            raise KeyError("Frame.contents has no key: %r" % (key,))
        if key == "header":
            self.header = None
    ...
    def render(self, size, focus=False):
        ...
        if ftrim + htrim < maxrow:
            body = self.body.render(
                (maxcol, maxrow - ftrim - htrim), focus and self.focus_part == "body"
            )
    ...
    def mouse_event(self, size, event, button, col, row, focus):
        ...
        if row >= maxrow - ftrim:  # within footer
            ...
            return self.footer.mouse_event(
                (maxcol,), event, button, col, row - maxrow + ftrim, focus
            )
...
class Pile(Widget, WidgetContainerMixin, WidgetContainerListContentsMixin):
        ...
        *widget_list* may also contain tuples such as:
        ...
        for i, original in enumerate(widget_list):
            ...
            if not isinstance(w, tuple):
                ...
                raise PileError("initial widget list item invalid %r" % (original,))
    ...
    def get_item_rows(self, size, focus):
        ...
        if wtotal == 0:
            raise PileError(
                "No weighted widgets found for Pile treated as a box widget"
            )
...
class Columns(Widget, WidgetContainerMixin, WidgetContainerListContentsMixin):
        ...
        *box_columns* will be displayed with this calculated number of rows,
        ...
        for i, original in enumerate(widget_list):
            ...
            if not isinstance(w, tuple):
                ...
                raise ColumnsError("initial widget list item invalid: %r" % (original,))
    ...
    focus = property(
        get_focus, doc="the child widget in focus or None when Columns is empty"
    )
    ...
    def get_cursor_coords(self, size):
        ...
        x, y = coords
        x += sum(
            [self.dividechars + wc for wc in widths[: self.focus_position] if wc > 0]
        )
        return x, y
        ...
        for i, (width, (w, (t, n, b))) in enumerate(zip(widths, self.contents)):
            ...
            if len(size) == 1 and b:
                return w.mouse_event(
                    (end - x, self.rows(size)), event, button, col - x, row, focus
                )
            return w.mouse_event(
                (end - x,) + size[1:], event, button, col - x, row, focus
            )
        return False
