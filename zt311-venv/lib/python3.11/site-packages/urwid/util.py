...
def apply_target_encoding(s):
    ...
    if _use_dec_special and type(s) == text_type:
        ...
        try:
            ...
            for c, alt in zip(escape.DEC_SPECIAL_CHARS, escape.ALT_DEC_SPECIAL_CHARS):
...
def trim_text_attr_cs(text, attr, cs, start_col, end_col):
    ...
    return (
        bytes().rjust(pad_left) + text[spos:epos] + bytes().rjust(pad_right),
        attrtr,
        cstr,
    )
...
def _tagmarkup_recurse(tm, attr):
    ...
    if type(tm) == tuple:
        ...
        if len(tm) != 2:
            raise TagMarkupException(
                "Tuples must be in the form (attribute, tagmarkup): %r" % (tm,)
            )
