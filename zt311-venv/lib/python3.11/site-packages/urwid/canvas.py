...
class Canvas(object):
    ...
    def _text_content(self):
        ...
        """
        return [
            bytes().join([text for (attr, cs, text) in row]) for row in self.content()
        ]
...
class TextCanvas(Canvas):
    ...
    def __init__(
        self, text=None, attr=None, cs=None, cursor=None, maxcol=None, check_width=True
    ):
        """
...
class CompositeCanvas(Canvas):
    ...
    def __init__(self, canv=None):
        ...
        if canv is None:
            ...
            if hasattr(canv, "shards"):
                ...
                self.shards = [
                    (canv.rows(), [(0, 0, canv.cols(), canv.rows(), None, canv)])
                ]
    ...
    def trim(self, top, count=None):
        ...
        assert top < self.rows(), "cannot trim %d lines from %d!" % (top, self.rows())
        if self.widget_info:
            raise self._finalized_error
    ...
    def trim_end(self, end):
        ...
        assert end <= self.rows(), "cannot trim %d lines from %d!" % (end, self.rows())
        if self.widget_info:
            raise self._finalized_error
    ...
    def overlay(self, other, left, top):
        ...
        assert right >= 0, "top canvas of overlay not the size expected!" + repr(
            (other.cols(), left, right, width)
        )
        assert bottom >= 0, "top canvas of overlay not the size expected!" + repr(
            (other.rows(), top, bottom, height)
        )
...
def CanvasJoin(l):
    ...
    if focus_item:
        children = (
            [children[focus_item]] + children[:focus_item] + children[focus_item + 1 :]
        )
