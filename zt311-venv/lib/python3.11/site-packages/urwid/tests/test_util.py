...
class ConvertDecSpecialTest(unittest.TestCase):
    def ctest(self, desc, s, exp, expcs):
        ...
        assert result == exp, "%s got:%r expected:%r" % (desc, result, exp)
        resultcs = c._cs[0]
        assert resultcs == expcs, "%s got:%r expected:%r" % (desc, resultcs, expcs)
...
class WithinDoubleByteTest(unittest.TestCase):
    ...
    def wtest(self, s, ls, pos, expected, desc):
        ...
        assert result == expected, "%s got:%r expected: %r" % (desc, result, expected)
...
class CalcTextPosTest(unittest.TestCase):
    def ctptest(self, text, tests):
        ...
        for s, e, p, expected in tests:
            ...
            assert got == expected, "%r got:%r expected:%r" % ((s, e, p), got, expected)
...
class TagMarkupTest(unittest.TestCase):
    ...
    def test_bad_tuple(self):
        self.assertRaises(
            urwid.TagMarkupException, lambda: urwid.decompose_tagmarkup((1, 2, 3))
        )

    def test_bad_type(self):
        self.assertRaises(
            urwid.TagMarkupException, lambda: urwid.decompose_tagmarkup(5)
        )
