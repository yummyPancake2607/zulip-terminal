...
class SubsegTest(unittest.TestCase):
    ...
    def test2_text(self):
        ...
        self.st(
            (10, 0, B("12\xA1\xA156\xA1\xA190")), "", 3, 8, [(5, 0, B(" 56\xA1\xA1"))]
        )
        self.st(
            (10, 0, B("12\xA1\xA156\xA1\xA190")), "", 2, 7, [(5, 0, B("\xA1\xA156 "))]
        )
        self.st((10, 0, B("12\xA1\xA156\xA1\xA190")), "", 3, 7, [(4, 0, B(" 56 "))])
        self.st(
            (10, 0, B("12\xA1\xA156\xA1\xA190")),
            "",
            0,
...
class CalcTranslateTest(object):
    ...
    def test1_left(self):
        result = urwid.default_layout.layout(self.text, self.width, "left", self.mode)
        assert result == self.result_left, result

    def test2_right(self):
        result = urwid.default_layout.layout(self.text, self.width, "right", self.mode)
        assert result == self.result_right, result

    def test3_center(self):
        result = urwid.default_layout.layout(self.text, self.width, "center", self.mode)
...
class CalcPosTest(unittest.TestCase):
    ...
    def tests(self):
        for x, y, expected in self.mytests:
            ...
            assert got == expected, "%r got:%r expected:%r" % ((x, y), got, expected)
