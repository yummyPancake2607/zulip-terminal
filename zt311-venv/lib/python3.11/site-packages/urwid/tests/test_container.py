...
class FrameTest(unittest.TestCase):
    def ftbtest(
        self, desc, focus_part, header_rows, footer_rows, size, focus, top, bottom
    ):
        class FakeWidget:
        ...
        assert exp == rval, "%s expected %r but got %r" % (desc, exp, rval)
...
class PileTest(unittest.TestCase):
    def ktest(self, desc, l, focus_item, key, rkey, rfocus, rpref_col):
        p = urwid.Pile(l, focus_item)
        rval = p.keypress((20,), key)
        assert rkey == rval, "%s key expected %r but got %r" % (desc, rkey, rval)
        ...
        assert new_pref == rpref_col, "%s pref_col expected %r but got %r" % (
            desc,
            rpref_col,
            new_pref,
        )
    ...
    def test_select_change(self):
        ...
        self.ktest("ignore down", [S(""), T("")], 0, "down", "down", 0, 0)
        self.ktest("step up", [S(""), S("")], 1, "up", None, 0, 0)
        self.ktest("step down", [S(""), S("")], 0, "down", None, 1, 0)
        self.ktest("skip step up", [S(""), T(""), S("")], 2, "up", None, 0, 0)
        self.ktest("skip step down", [S(""), T(""), S("")], 0, "down", None, 2, 0)
        self.ktest(
            "pad skip step up", [T(""), S(""), T(""), S("")], 3, "up", None, 1, 0
        )
        self.ktest(
            "pad skip step down", [S(""), T(""), S(""), T("")], 0, "down", None, 2, 0
        )
        self.ktest(
            "padi skip step up",
            [S(""), T(""), S(""), T(""), S("")],
        ...
        self.ktest("pref step up", [S(""), T(""), e], 2, "up", None, 0, 2)
        self.ktest("pref step down", [e, T(""), S("")], 0, "down", None, 2, 2)
        z = E("", "1234")
        self.ktest("prefx step up", [z, T(""), e], 2, "up", None, 0, 2)
        assert z.get_pref_col((20,)) == 2
        z = E("", "1234")
        self.ktest("prefx step down", [e, T(""), z], 0, "down", None, 2, 2)
        assert z.get_pref_col((20,)) == 2
...
class ColumnsTest(unittest.TestCase):
    ...
    def test_widths(self):
        ...
        self.cwtest(
            "mixed 4",
            [("weight", 2, x), ("fixed", 5, x), x, ("weight", 3, x)],
            1,
            (14,),
            [2, 5, 1, 3],
        )
    ...
    def mctest(self, desc, l, divide, size, col, row, exp, f_col, pref_col):
        ...
        assert pc == pref_col, "%s expected pref_col %s, got %s" % (desc, pref_col, pc)
...
class CommonContainerTest(unittest.TestCase):
    ...
    def test_grid_flow(self):
        ...
        self.assertRaises(
            urwid.GridFlowError, lambda: gf.contents.insert(1, (t1, ("pack", None)))
        )
        gf.focus_position = 0
    ...
    def test_overlay(self):
        ...
        o = urwid.Overlay(
            s1, s2, "center", ("relative", 50), "middle", ("relative", 50)
        )
