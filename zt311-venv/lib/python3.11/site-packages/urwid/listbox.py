...
class SimpleListWalker(MonitoredList, ListWalker):
    def __init__(self, contents, wrap_around=False):
        ...
        if not getattr(contents, "__getitem__", None):
            raise ListWalkerError(
                "SimpleListWalker expecting list like object, got: %r" % (contents,)
            )
...
class SimpleFocusListWalker(ListWalker, MonitoredFocusList):
    def __init__(self, contents, wrap_around=False):
        ...
        if not getattr(contents, "__getitem__", None):
            raise ListWalkerError(
                "SimpleFocusListWalker expecting list like "
...
class ListBox(Widget, WidgetContainerMixin):
    ...
    def render(self, size, focus=False):
        ...
        for widget, w_pos, w_rows in fill_above:
            ...
            if w_rows != canvas.rows():
                raise ListBoxError(
                    "Widget %r at position %r within listbox calculated %d rows but rendered %d!"
        ...
        if focus_canvas.rows() != focus_rows:
            raise ListBoxError(
                "Focus Widget %r at position %r within listbox calculated %d rows but rendered %d!"
                % (focus_widget, focus_pos, focus_rows, focus_canvas.rows())
            )
        c_cursor = focus_canvas.cursor
        if cursor is not None and cursor != c_cursor:
            raise ListBoxError(
                "Focus Widget %r at position %r within listbox calculated cursor coords %r but rendered cursor coords %r!"
        ...
        if rows > maxrow:
            raise ListBoxError(
                "Listbox contents too long!  Probably urwid's fault (please report): %r"
                % ((top, middle, bottom),)
            )
    ...
    def set_focus(self, position, coming_from=None):
        ...
        if coming_from not in ("above", "below", None):
            raise ListBoxError("coming_from value invalid: %r" % (coming_from,))
        focus_widget, focus_pos = self._body.get_focus()
    ...
    focus = property(
        _get_focus, doc="the child widget in focus or None when ListBox is empty"
    )
    ...
    def _set_focus_valign_complete(self, size, focus):
        ...
        rtop, rbot = calculate_top_bottom_filler(
            maxrow, vt, va, GIVEN, rows, None, 0, 0
        )
    ...
    def shift_focus(self, size, offset_inset):
        ...
        if offset_inset >= 0:
            if offset_inset >= maxrow:
                raise ListBoxError(
                    "Invalid offset_inset: %r, only %r rows in list box"
                    % (offset_inset, maxrow)
                )
            ...
            if offset_inset + tgt_rows <= 0:
                raise ListBoxError(
                    "Invalid offset_inset: %r, only %r rows in target!"
                    % (offset_inset, tgt_rows)
                )
            self.offset_rows = 0
    ...
    def change_focus(
        ...
        if offset_inset >= 0:
            ...
            if offset_inset + tgt_rows <= 0:
                raise ListBoxError(
                    "Invalid offset_inset: %s, only %s rows in target!"
                    % (offset_inset, tgt_rows)
                )
            self.offset_rows = 0
            self.inset_fraction = (-offset_inset, tgt_rows)
        ...
        if len(cursor_coords) == 1:
            ...
            if pref_row < 0 or pref_row >= tgt_rows:
                raise ListBoxError(
                    "cursor_coords row outside valid range for target. pref_row:%r target_rows:%r"
                    % (pref_row, tgt_rows)
                )
    ...
    def _keypress_page_up(self, size):
        ...
        search_order = list(xrange(snap_region_start, len(t))) + list(
            xrange(snap_region_start - 1, -1, -1)
        )
        ...
        for i in good_choices + search_order:
            ...
            self.change_focus(
                (maxcol, maxrow), pos, row_offset, "below", None, snap_rows
            )
            return
        ...
        rows = widget.rows((maxcol,), True)
        self.change_focus(
            (maxcol, maxrow), pos, -(rows - 1), "below", (self.pref_col, rows - 1), 0
        )
    ...
    def _keypress_page_down(self, size):
        ...
        for i in good_choices + search_order:
            ...
            self.change_focus(
                (maxcol, maxrow), pos, row_offset, "above", None, snap_rows
            )
            return
        ...
        rows = widget.rows((maxcol,), True)
        self.change_focus(
            (maxcol, maxrow), pos, maxrow - 1, "above", (self.pref_col, 0), 0
        )
