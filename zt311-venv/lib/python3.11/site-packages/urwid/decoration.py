...
class BoxAdapter(WidgetDecoration):
    ...
    def __init__(self, box_widget, height):
        ...
        if hasattr(box_widget, "sizing") and BOX not in box_widget.sizing():
            raise BoxAdapterError("%r is not a box widget" % box_widget)
    ...
    box_widget = property(
        WidgetDecoration._get_original_widget, WidgetDecoration._set_original_widget
    )
    ...
    def mouse_event(self, size, event, button, col, row, focus):
        ...
        return self._original_widget.mouse_event(
            (maxcol, self.height), event, button, col, row, focus
        )
...
class Padding(WidgetDecoration):
    def __init__(
        self, w, align=LEFT, width=RELATIVE_100, min_width=None, left=0, right=0
    ):
        """
        ...
        if type(align) == tuple and align[0] in ("fixed left", "fixed right"):
            if align[0] == "fixed left":
                ...
                right = align[1]
                align = RIGHT
        if type(width) == tuple and width[0] in ("fixed left", "fixed right"):
    ...
    def mouse_event(self, size, event, button, x, y, focus):
        ...
        maxvals = (maxcol - left - right,) + size[1:]
        return self._original_widget.mouse_event(
            maxvals, event, button, x - left, y, focus
        )
...
class Filler(WidgetDecoration):
    def __init__(
        self, body, valign=MIDDLE, height=PACK, min_height=None, top=0, bottom=0
    ):
        """
        ...
        if isinstance(height, tuple):
            if height[0] == "fixed top":
                if not isinstance(valign, tuple) or valign[0] != "fixed bottom":
                    raise FillerError(
                        "fixed top height may only be used " "with fixed bottom valign"
                    )
                top = height[1]
                height = RELATIVE_100
                ...
                if not isinstance(valign, tuple) or valign[0] != "fixed top":
                    raise FillerError(
                        "fixed bottom height may only be used " "with fixed top valign"
                    )
                bottom = height[1]
                height = RELATIVE_100
    ...
    def mouse_event(self, size, event, button, col, row, focus):
        ...
        if self.height_type == PACK:
            return self._original_widget.mouse_event(
                (maxcol,), event, button, col, row - top, focus
            )
        return self._original_widget.mouse_event(
            (maxcol, maxrow - top - bottom), event, button, col, row - top, focus
        )
...
def normalize_align(align, err):
    ...
    if align in (LEFT, CENTER, RIGHT):
        ...
        return align
    raise err(
        "align value %r is not one of 'left', 'center', "
        "'right', ('relative', percentage 0=left 100=right)" % (align,)
    )
...
def normalize_width(width, err):
    ...
    if width in (CLIP, PACK):
        ...
        return width
    raise err(
        "width value %r is not one of fixed number of columns, "
        "'pack', ('relative', percentage of total width), 'clip'" % (width,)
    )
...
def normalize_valign(valign, err):
    ...
    if valign in (TOP, MIDDLE, BOTTOM):
        ...
        return valign
    raise err(
        "valign value %r is not one of 'top', 'middle', "
        "'bottom', ('relative', percentage 0=left 100=right)" % (valign,)
    )
...
def normalize_height(height, err):
    ...
    if height in (FLOW, PACK):
        ...
        return (GIVEN, height)
    raise err(
        "height value %r is not one of fixed number of columns, "
        "'pack', ('relative', percentage of total height)" % (height,)
    )
...
def calculate_left_right_padding(
    maxcol, align_type, align_amount, width_type, width_amount, min_width, left, right
):
    """
