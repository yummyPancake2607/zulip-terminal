...
class Screen(BaseScreen, RealTerminal):
    def __init__(self, input=sys.stdin, output=sys.stdout):
        ...
        signals.connect_signal(
            self, UPDATE_PALETTE_ENTRY, self._on_update_palette_entry
        )
    ...
    def _last_row(self, row):
        ...
        last_offs, z_col = util.calc_text_pos(
            last_text, 0, len(last_text), last_cols - 1
        )
        if last_offs == 0:
            z_text = last_text
            del new_row[-1]
            ...
            z_col += nlast_cols
            nlast_offs, y_col = util.calc_text_pos(
                nlast_text, 0, len(nlast_text), nlast_cols - 1
            )
            ...
            nlast_offs, y_col = util.calc_text_pos(
                last_text, 0, last_offs, nlast_cols - 1
            )
    ...
    def set_terminal_properties(
        self, colors=None, bright_is_bold=None, has_underline=None
    ):
        """
        ...
        if (
            colors == self.colors
            and bright_is_bold == self.fg_bright_is_bold
