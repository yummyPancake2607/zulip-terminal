...
def validate_size(widget, size, canv):
    ...
    if (size and size[1:] != (0,) and size[0] != canv.cols()) or (
        ...
        raise WidgetError(
            "Widget %r rendered (%d x %d) canvas"
            " when passed size %r!" % (widget, canv.cols(), canv.rows(), size)
        )
...
class Widget(with_metaclass(WidgetMeta, object)):
        ...
        if not size:
            if FIXED in self.sizing():
                raise NotImplementedError(
                    "Fixed widgets must override" " Widget.pack()"
                )
            raise WidgetError(
                "Cannot pack () size, this is not a fixed" " widget: %s" % repr(self)
            )
        elif len(size) == 1:
            if FLOW in self.sizing():
                return size + (self.rows(size, focus),)
            raise WidgetError(
                "Cannot pack (maxcol,) size, this is not a"
                " flow widget: %s" % repr(self)
            )
        return size

    base_widget = property(
        lambda self: self,
        doc="""
    ...
    def _not_a_container(self, val=None):
        raise IndexError("No focus_position, %r is not a container widget" % self)
...
def fixed_size(size):
    ...
    """
    if size != ():
        raise ValueError("FixedWidget takes only () for size." "passed: %r" % (size,))
...
class Divider(Widget):
    ...
    def _repr_words(self):
        return self.__super._repr_words() + [python3_repr(self.div_char)] * (
            self.div_char != " "
        )
...
class Text(Widget):
    ...
    def _repr_attrs(self):
        attrs = dict(
            self.__super._repr_attrs(), align=self._align_mode, wrap=self._wrap_mode
        )
    ...
    def set_align_mode(self, mode):
        ...
        if not self.layout.supports_align_mode(mode):
            raise TextError("Alignment mode %r not supported." % (mode,))
...
class Edit(Text):
    ...
    * ``"postchange"`` is sent after the value of edit_text changes.
    ...
    def _repr_attrs(self):
        attrs = dict(self.__super._repr_attrs(), edit_pos=self._edit_pos)
    ...
    def set_text(self, markup):
        ...
        raise EditError(
            "set_text() not supported.  Use set_caption()"
    ...
    def update_text(self):
        ...
        """
        raise EditError(
            "update_text() has been removed.  Use "
    ...
    def get_line_translation(self, maxcol, ta=None):
        ...
        if x < 0:
            ...
            return (
                trans[:y] + [shift_line(trans[y], -(x - maxcol + 1))] + trans[y + 1 :]
            )
        return trans
...
def delegate_to_widget_mixin(attribute_name):
    ...
    class DelegateToWidgetMixin(Widget):
        ...
        get_cursor_coords = property(lambda self: get_delegate(self).get_cursor_coords)
...
class WidgetWrap(delegate_to_widget_mixin("_wrapped_widget"), Widget):
    ...
    def _raise_old_name_error(self, val=None):
        raise WidgetWrapError(
            "The WidgetWrap.w member variable has "
