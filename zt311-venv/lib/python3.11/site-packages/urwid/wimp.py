...
from urwid.widget import Text, WidgetWrap, delegate_to_widget_mixin, BOX, FLOW
...
class CheckBox(WidgetWrap):
    ...
    def _repr_attrs(self):
        return dict(self.__super._repr_attrs(), state=self.state)
    ...
    def set_state(self, state, do_callback=True):
        ...
        if state not in self.states:
            raise CheckBoxError("%s Invalid state: %s" % (repr(self), repr(state)))
...
class RadioButton(CheckBox):
    ...
    def __init__(
        self, group, label, state="first True", on_state_change=None, user_data=None
    ):
        """
        ...
        self.group = group
        self.__super.__init__(label, state, False, on_state_change, user_data)
        group.append(self)
