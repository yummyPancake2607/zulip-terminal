    ...
    def start(self):
        ...
        try:
            signals.connect_signal(
                self.screen, INPUT_DESCRIPTORS_CHANGED, self._reset_input_descriptors
            )
        except NameError:
            pass
            ...
            if is_mouse_event(k):
                ...
                if hasattr(self._topmost_widget, "mouse_event"):
                    if self._topmost_widget.mouse_event(
                        self.screen_size, event, button, col, row, focus=True
                    ):
                        k = None
    ...
    def _loop(self):
        ...
        if self._alarms or self._did_something:
            ...
            if self._did_something and (
                not self._alarms or (self._alarms and timeout > 0)
            ):
                timeout = 0
                tm = "idle"
...
class GLibEventLoop(EventLoop):
    ...
    def set_signal_handler(self, signum, handler):
        ...
        source = self.GLib.unix_signal_add(
            self.GLib.PRIORITY_DEFAULT, signum, final_handler, signum
        )
