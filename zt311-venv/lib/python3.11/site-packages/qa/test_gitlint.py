...
class IntegrationTests(BaseTestCase):
    ...
    def test_successful_merge_commit(self):
        ...
        self.assertEqualStdout(
            output,
            f"1: T1 Title exceeds max length (90>72): \"Merge '{commit_title}'\"\n",
        )
    ...
    def test_fixup_commit(self):
        ...
        with open(
            os.path.join(self.tmp_git_repo, test_filename),
            "a",
    ...
    def test_fixup_amend_commit(self):
        ...
        git(
            "commit",
            "--no-edit",
            f"--fixup=amend:{self.get_last_commit_hash()}",
            _cwd=self.tmp_git_repo,
        )
    ...
    def test_violations(self):
        ...
        self.assertEqualStdout(
            output, self.get_expected("test_gitlint/test_violations_1")
        )
    ...
    def test_msg_filename_no_tty(self):
        ...
        self.assertEqualStdout(
            output, self.get_expected("test_gitlint/test_msg_filename_no_tty_1")
        )
    ...
    def test_git_empty_repo(self):
        ...
        output = gitlint(
            _cwd=empty_git_repo, _tty_in=True, _ok_code=[self.GIT_CONTEXT_ERROR_CODE]
        )
    ...
    def test_commit_binary_file(self):
        ...
        binary_filename = self.create_simple_commit(
            "Sïmple commit", file_contents=bytes([0x48, 0x00, 0x49, 0x00])
        )
        output = gitlint(
            "--debug",
            _ok_code=1,
        ...
        expected_kwargs.update(
            {
                ...
                "changed_files_stats": (
                    f"{binary_filename}: None additions, None deletions"
                ),
            }
        )
        expected = self.get_expected(
            "test_gitlint/test_commit_binary_file_1", expected_kwargs
        )
        self.assertEqualStdout(output, expected)
