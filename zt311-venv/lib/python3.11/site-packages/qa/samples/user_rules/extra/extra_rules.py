...
class GitCommitRule(CommitRule):
    ...
    def validate(self, commit):
        violations = [
            ...
            RuleViolation(
                self.id, f"GitCommit.custom_prop: {commit.custom_prop}", line_nr=1
            ),
        ]
...
class GitlintConfigurationRule(ConfigurationRule):
    ...
    def apply(self, config, commit):
        ...
        commit.message.body.append(
            f"{config.target} "
        )  # trailing whitespace deliberate to trigger violation
...
class ConfigurableCommitRule(CommitRule):
    ...
    def validate(self, _):
        violations = [
            RuleViolation(
                self.id, f"int-öption: {self.options[u'int-öption'].value}", line_nr=1
            ),
            RuleViolation(
                self.id, f"str-öption: {self.options[u'str-öption'].value}", line_nr=1
            ),
            RuleViolation(
                self.id, f"list-öption: {self.options[u'list-öption'].value}", line_nr=1
            ),
        ]
