...
class CommitsTests(BaseTestCase):
    ...
    def test_violations(self):
        ...
        self.assertEqualStdout(
            output, self.get_expected("test_commits/test_violations_1", expected_kwargs)
        )
    ...
    def test_csv_hash_list(self):
        ...
        expected_kwargs = {
            ...
            "commit_sha4": commit_sha4,
        }
        self.assertEqualStdout(
            output,
            self.get_expected("test_commits/test_csv_hash_list_1", expected_kwargs),
        )
    ...
    def test_lint_single_commit(self):
        ...
        output = gitlint(
            "--commit", commit_sha, _cwd=self.tmp_git_repo, _tty_in=True, _ok_code=[2]
        )
        ...
        output = gitlint(
            "--commits", refspec, _cwd=self.tmp_git_repo, _tty_in=True, _ok_code=[2]
        )
        self.assertEqual(output.exit_code, 2)
        self.assertEqualStdout(output, expected)
        ...
        output = gitlint(
            "--commits", refspec, _cwd=self.tmp_git_repo, _tty_in=True, _ok_code=[254]
        )
    ...
    def test_lint_staged_stdin(self):
        ...
        if matches:
            expected_date = arrow.get(
                str(matches.group(1)), "YYYY-MM-DD HH:mm:ss Z"
            ).format("YYYY-MM-DD HH:mm:ss Z")
            expected_kwargs["staged_date"] = expected_date

        self.assertEqualStdout(
            output,
            self.get_expected("test_commits/test_lint_staged_stdin_1", expected_kwargs),
        )
    ...
    def test_lint_staged_msg_filename(self):
        ...
        expected = self.get_expected(
            "test_commits/test_lint_staged_msg_filename_1", expected_kwargs
        )
        self.assertEqualStdout(output, expected)
        self.assertEqual(output.exit_code, 3)
    ...
    def test_lint_head(self):
        ...
        output = gitlint(
            "--commits", "HEAD", _cwd=tmp_git_repo, _tty_in=True, _ok_code=[3]
        )
        ...
        self.assertEqualStdout(
            output, self.get_expected("test_commits/test_lint_head_1", expected_kwargs)
        )
    ...
    def test_ignore_commits(self):
        ...
        self.create_simple_commit(
            "Sïmple title4.\n\nSimple bödy describing the commit4",
            git_repo=tmp_git_repo,
        )
        revlist = git("rev-list", "HEAD", _tty_in=True, _cwd=tmp_git_repo).split()
        ...
        self.assertEqualStdout(
            output,
            self.get_expected("test_commits/test_ignore_commits_1", expected_kwargs),
        )
