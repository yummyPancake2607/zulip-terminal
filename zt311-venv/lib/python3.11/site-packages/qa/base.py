...
class BaseTestCase(TestCase):
    ...
    def get_last_commit_short_hash(self, git_repo=None):
        git_repo = self.tmp_git_repo if git_repo is None else git_repo
        return git(
            "rev-parse", "--short", "HEAD", _cwd=git_repo, _err_to_out=True
        ).replace("\n", "")
    ...
    def get_system_info_dict():
        ...
        expected_gitlint_version = (
            gitlint("--version").replace("gitlint, version ", "").strip()
        )
    ...
    def get_debug_vars_last_commit(self, git_repo=None):
        ...
        expected_date = git(
            "log", "-1", "--pretty=%ai", _tty_out=False, _cwd=target_repo
        )
        expected_date = arrow.get(str(expected_date), "YYYY-MM-DD HH:mm:ss Z").format(
