...
class TestTagsApi:
    ...
    def test_add_user_tags(self):
        ...
        self.cli.api.add_user_tag(
            self.user_id, self.room_id, "foo", body={"order": "5"}
        )
        req = responses.calls[0].request
        assert req.url == tags_url
...
class TestAccountDataApi:
    ...
    def test_set_room_account_data(self):
        account_data_url = (
            "http://example.com/_matrix/client/r0/user"
            "/@user:matrix.org/rooms/#foo:matrix.org/account_data/foo"
        )
        responses.add(responses.PUT, account_data_url, body="{}")
        self.cli.api.set_room_account_data(
            self.user_id, self.room_id, "foo", {"bar": 1}
        )
        req = responses.calls[0].request
        assert req.url == account_data_url
        assert req.method == "PUT"
...
class TestUnbanApi:
    ...
    def test_unban(self):
        unban_url = (
            "http://example.com" "/_matrix/client/r0/rooms/#foo:matrix.org/unban"
        )
...
class TestKeysApi:
    ...
    device_keys = {
        ...
        "signatures": {
            "@alice:example.com": {
                "ed25519:JLAFKJWSCS": (
                    "dSO80A01XiigH3uBiDVx/EjzaoycHcjq9lfQX0uWsqxl2gi"
    ...
    @pytest.mark.parametrize(
        "args", [{}, {"device_keys": device_keys}, {"one_time_keys": one_time_keys}]
    )
...
class TestMainApi:
    user_id = "@alice:matrix.org"
    token = (
        "Dp0YKRXwx0iWDhFj7lg3DVjwsWzGcUIgARljgyAip2JD8qd5dSaW"
    ...
    def test_send_token_query(self):
        mapi = api.MatrixHttpApi(
            "http://example.com", token=self.token, use_authorization_header=False
        )
        responses.add(
            responses.GET,
            mapi._base_url + MATRIX_V2_API_PATH + self.test_path,
    ...
    def test_send_user_id(self):
        mapi = api.MatrixHttpApi(
            "http://example.com", token=self.token, identity=self.user_id
        )
        responses.add(
            responses.GET,
            mapi._base_url + MATRIX_V2_API_PATH + self.test_path,
...
class TestMediaApi:
    ...
    def test_media_download(self):
        media_url = "http://example.com/_matrix/media/r0/download/" + self.mxcurl[6:]
    ...
    def test_get_thumbnail(self):
        media_url = "http://example.com/_matrix/media/r0/thumbnail/" + self.mxcurl[6:]
        with open("test/response_examples.py", "rb") as fil:
            responses.add(
                responses.GET,
                media_url,
        ...
        resp = self.cli.api.get_thumbnail(self.mxcurl, 28, 28, allow_remote=False)
        resp.raw.decode_content = True
        req = responses.calls[0].request
        assert req.url.split("?")[0] == media_url
        assert req.method == "GET"
    ...
    def test_get_url_preview(self):
        media_url = "http://example.com/_matrix/media/r0/preview_url"
        preview_url = deepcopy(response_examples.example_preview_url)
        responses.add(responses.GET, media_url, body=json.dumps(preview_url))
...
class TestRoomApi:
    ...
    def test_create_room_visibility_public(self):
        create_room_url = "http://example.com" "/_matrix/client/r0/createRoom"
        responses.add(
            responses.POST,
            create_room_url,
            json='{"room_id": "!sefiuhWgwghwWgh:example.com"}',
        )
        self.cli.api.create_room(name="test", alias="#test:example.com", is_public=True)
        req = responses.calls[0].request
    ...
    def test_create_room_visibility_private(self):
        ...
        self.cli.api.create_room(
            name="test", alias="#test:example.com", is_public=False
        )
        req = responses.calls[0].request
        assert req.url == create_room_url
        assert req.method == "POST"
    ...
    def test_create_room_federate_true(self):
        ...
        self.cli.api.create_room(
            name="test2", alias="#test2:example.com", federate=True
        )
        req = responses.calls[0].request
        assert req.url == create_room_url
        assert req.method == "POST"
    ...
    def test_create_room_federate_false(self):
        ...
        self.cli.api.create_room(name="test", alias="#test:example.com", federate=False)
        req = responses.calls[0].request
        assert req.url == create_room_url
        assert req.method == "POST"
        j = json.loads(req.body)
...
class TestWhoamiQuery:
    ...
    def test_whoami(self):
        ...
        responses.add(
            responses.GET, whoami_url, body='{"user_id": "%s"}' % self.user_id
        )
        mapi.whoami()
        req = responses.calls[0].request
        assert req.method == "GET"
