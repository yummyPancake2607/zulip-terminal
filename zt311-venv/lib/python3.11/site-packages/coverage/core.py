    ...
    def __init__(
        ...
        *,
        ...
        if not env.PYBEHAVIOR.pep669:
            ...
            (bad := c) in config.concurrency for c in ["greenlet", "eventlet", "gevent"]
        ):
        ...
        if core_name == "sysmon" and reason_no_sysmon:
            ...
            warn(
                f"Can't use core=sysmon: {reason_no_sysmon}, using default core",
                slug="no-sysmon",
            )
            core_name = None
        ...
        if core_name == "ctrace":
            if not CTRACER_FILE:
                ...
                core_name = "pytrace"
                _debug(
                    "core.py: Falling back to pytrace because C tracer not available"
                )
