...
class HookTests(BaseTestCase):
    ...
    def test_commit_msg_hook_path(self, git_hooks_dir):
        ...
        expected_path = os.path.join(
            git_hooks_dir.return_value, COMMIT_MSG_HOOK_DST_PATH
        )
    ...
    def test_install_commit_msg_hook(
        git_hooks_dir, isdir, path_exists, copy, stat, chmod
    ):
        lint_config = LintConfig()
    ...
    @patch("os.path.isdir", return_value=True)
    @patch("gitlint.hooks.git_hooks_dir")
    def test_install_commit_msg_hook_negative(
        self, git_hooks_dir, isdir, path_exists, copy
    ):
        lint_config = LintConfig()
        lint_config.target = os.path.join("/hür", "dur")
        git_hooks_dir.return_value = os.path.join("/föo", "bar", ".git", "hooks")
    ...
    @patch("os.path.isdir", return_value=True)
    @patch("gitlint.hooks.git_hooks_dir")
    def test_uninstall_commit_msg_hook_negative(
        self, git_hooks_dir, isdir, path_exists, remove
    ):
        lint_config = LintConfig()
        lint_config.target = os.path.join("/hür", "dur")
        git_hooks_dir.return_value = os.path.join("/föo", "bar", ".git", "hooks")
