...
class LintTests(BaseTestCase):
    ...
    def test_lint_sample5(self):
        ...
        expected = [
            ...
            RuleViolation("T4", "Title contains hard tab characters (\\t)", title, 1),
            RuleViolation(
                "T5", "Title contains the word 'WIP' (case-insensitive)", title, 1
            ),
            RuleViolation(
                "B4", "Second line is not empty", "This line should be ëmpty", 2
            ),
            RuleViolation(
                "B2",
    ...
    def test_lint_ignore(self):
        ...
        violations = linter.lint(
            self.gitcommit(self.get_sample("commit_message/sample3"))
        )

        expected = [
            RuleViolation(
                "B4", "Second line is not empty", "This line should be empty", 2
            ),
            RuleViolation(
                "B3",
                "Line contains hard tab characters (\\t)",
    ...
    def test_lint_regex_rules(self):
        ...
        rule_regexes = [
            ("title-match-regex", "Tïtle$"),
            ("body-match-regex", "Sïgned-Off-By: (.*)$"),
        ]
        ...
        expected_violations = [
            RuleViolation(
                "T7", "Title does not match regex (^Tïtle)", "Normal Commit Tïtle", 1
            ),
            RuleViolation(
                "B8", "Body does not match regex (Sügned-Off-By: (.*)$)", None, 6
            ),
        ]
        violations = linter.lint(commit)
    ...
    def test_named_rules(self):
        ...
        violations = [
            ...
            RuleViolation(
                "T5:another-rule-ïd",
                "Title contains the word 'Föo' (case-insensitive)",
                "WIP: Föo bar",
                1,
            ),
            RuleViolation(
        ...
        self.assertListEqual(
            violations,
            linter.lint(self.gitcommit("WIP: Föo bar\n\nFoo bår hur dur bla bla")),
        )
