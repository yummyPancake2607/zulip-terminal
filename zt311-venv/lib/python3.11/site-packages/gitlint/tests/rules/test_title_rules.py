...
class TitleRuleTests(BaseTestCase):
    def test_max_line_length(self):
        ...
        expected_violation = RuleViolation(
            "T1", "Title exceeds max length (73>72)", "å" * 73
        )
        ...
        # assert raise on 121
        expected_violation = RuleViolation(
            "T1", "Title exceeds max length (121>120)", "å" * 121
        )
    ...
    def test_hard_tabs(self):
        ...
        # contains hard tab
        expected_violation = RuleViolation(
            "T4", "Title contains hard tab characters (\\t)", "This is å\ttest"
        )
        violations = rule.validate("This is å\ttest", None)
        self.assertListEqual(violations, [expected_violation])
    ...
    def test_trailing_punctuation(self):
        ...
        for char in punctuation:
            line = (
                "This is å test" + char
            )  # note that make sure to include some unicode!
            gitcontext = self.gitcontext(line)
            expected_violation = RuleViolation(
                "T3", f"Title has trailing punctuation ({char})", line
            )
    ...
    def test_title_must_not_contain_word(self):
        ...
        expected_violation2 = RuleViolation(
            "T5",
            "Title contains the word 'test' (case-insensitive)",
            "WIP:This is å test",
        )
        ...
        self.assertListEqual(
            violations, [expected_violation, expected_violation2, expected_violation3]
        )
    ...
    def test_regex_matches(self):
        ...
        violations = rule.validate(commit.message.title, commit)
        expected_violation = RuleViolation(
            "T7", "Title does not match regex (^UÅ[0-9]*)", "US1234: åbc"
        )
        self.assertListEqual(violations, [expected_violation])
