...
class UserRuleTests(BaseTestCase):
    def test_find_rule_classes(self):
        ...
        expected_option = options.IntOption(
            "violation-count", 1, "Number of violåtions to return"
        )
    ...
    def test_rules_from_init_file(self):
        ...
        user_rule_path = self.get_sample_path(
            os.path.join("user_rules", "parent_package")
        )
        classes = find_rule_classes(user_rule_path)
        ...
        expected = [
            "<class 'my_commit_rules.MyUserCommitRule'>",
            "<class 'parent_package.InitFileRule'>",
        ]
    ...
    def test_assert_valid_rule_class_negative_id(self):
        for parent_class in [rules.LineRule, rules.CommitRule]:
            ...
            expected_msg = (
                "User-defined rule class 'MyRuleClass' must have an 'id' attribute"
            )
            with self.assertRaisesMessage(UserRuleError, expected_msg):
                assert_valid_rule_class(MyRuleClass)
            ...
            for letter in ["T", "R", "B", "M", "I"]:
                ...
                expected_msg = f"The id '{letter}' of 'MyRuleClass' is invalid. Gitlint reserves ids starting with R,T,B,M,I"
                with self.assertRaisesMessage(UserRuleError, expected_msg):
                    assert_valid_rule_class(MyRuleClass)
    ...
    def test_assert_valid_rule_class_negative_name(self):
        for parent_class in [rules.LineRule, rules.CommitRule]:
            ...
            expected_msg = (
                "User-defined rule class 'MyRuleClass' must have a 'name' attribute"
            )
            with self.assertRaisesMessage(UserRuleError, expected_msg):
                assert_valid_rule_class(MyRuleClass)
