...
def find_rule_classes(extra_path):
    ...
    for module in modules:
        ...
        try:
            ...
            raise rules.UserRuleError(
                f"Error while importing extra-path module '{module}': {e}"
            )
...
def assert_valid_rule_class(
    clazz, rule_type="User-defined"
    ...
    if not hasattr(clazz, "id") or clazz.id is None or not clazz.id:
        raise rules.UserRuleError(
            f"{rule_type} rule class '{clazz.__name__}' must have an 'id' attribute"
        )
    ...
    if not hasattr(clazz, "name") or clazz.name is None or not clazz.name:
        raise rules.UserRuleError(
            f"{rule_type} rule class '{clazz.__name__}' must have a 'name' attribute"
        )
    ...
    if issubclass(clazz, rules.LineRule) or issubclass(clazz, rules.CommitRule):
        if not hasattr(clazz, "validate") or not inspect.isroutine(clazz.validate):
            raise rules.UserRuleError(
                f"{rule_type} rule class '{clazz.__name__}' must have a 'validate' method"
            )
