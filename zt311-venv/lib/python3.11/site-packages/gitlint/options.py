...
class IntOption(RuleOption):
    ...
    def _raise_exception(self, value):
        if self.allow_negative:
            error_msg = (
                f"Option '{self.name}' must be an integer (current value: '{value}')"
            )
        else:
    ...
    def set(self, value):
        ...
        if value not in ["true", "false"]:
            raise RuleOptionError(
                f"Option '{self.name}' must be either 'true' or 'false'"
            )
    ...
    def set(self, value):
        ...
        if self.type == "dir":
            ...
            if not os.path.isdir(value) and not os.path.isfile(value):
                error_msg = f"Option {self.name} must be either an existing directory or file (current value: '{value}')"
        else:
