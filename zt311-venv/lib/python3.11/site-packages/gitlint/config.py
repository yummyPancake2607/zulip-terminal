...
from gitlint import (
    rules,
)  # For some weird reason pylint complains about this, pylint: disable=unused-import
...
class LintConfig:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self):
        ...
        self._target = options.PathOption(
            "target", os.path.realpath(os.getcwd()), target_description
        )
        self._ignore = options.ListOption("ignore", [], "List of rule-ids to ignore")
        self._contrib = options.ListOption(
            "contrib", [], "List of contrib-rules to enable"
        )
        self._config_path = None
        ignore_stdin_description = (
            "Ignore any stdin data. Useful for running in CI server."
        )
        self._ignore_stdin = options.BoolOption(
            "ignore-stdin", False, ignore_stdin_description
        )
        self._staged = options.BoolOption(
            "staged", False, "Read staged commit meta-info from the local repository."
        )
        ...
        self._regex_style_search = options.BoolOption(
            "regex-style-search",
            False,
            "Use `search` instead of `match` semantics for regex rules",
        )
    ...
    def contrib(self, value):
        try:
            ...
            for rule_id_or_name in self.contrib:
                rule_class = next(
                    (rc for rc in rule_classes if rule_id_or_name in (rc.id, rc.name)),
                    False,
                )
                ...
                if rule_class:
                    ...
                    raise LintConfigError(
                        f"No contrib rule with id or name '{rule_id_or_name}' found."
                    )
    ...
    def _get_option(self, rule_name_or_id, option_name):
        ...
        if not option:
            raise LintConfigError(
                f"Rule '{rule_name_or_id}' has no option '{option_name}'"
            )
...
class RuleCollection:
    ...
    def find_rule(self, rule_id_or_name):
        ...
        if not rule:
            rule = next(
                (rule for rule in self._rules.values() if rule.name == rule_id_or_name),
                None,
            )
        return rule
...
class LintConfigBuilder:
    ...
    def _add_named_rule(self, config, qualified_rule_name):
        ...
        canonical_name = (
            parent_rule.__class__.name + self.RULE_QUALIFIER_SYMBOL + rule_name
        )

        # Add the rule to the collection of rules if it's not there already
        if not config.rules.find_rule(canonical_id):
            config.rules.add_rule(
                parent_rule.__class__,
                canonical_id,
                {"is_named": True, "name": canonical_name},
            )
...
GITLINT_CONFIG_TEMPLATE_SRC_PATH = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "files/gitlint"
)
