...
class RuleViolation:
    ...
    def __eq__(self, other):
        ...
        equal = (
            equal and self.content == other.content and self.line_nr == other.line_nr
        )
        return equal
...
class MaxLineLength(LineRule):
    ...
    def validate(self, line, _commit):
        ...
        if len(line) > max_length:
            return [
                RuleViolation(
                    self.id, self.violation_message.format(len(line), max_length), line
                )
            ]
...
class LineMustNotContainWord(LineRule):
    ...
    options_spec = [
        ListOption(
            "words", [], "Comma separated list of words that should not be found"
        )
    ]
    ...
    def validate(self, line, _commit):
        ...
        for string in strings:
            ...
            if match:
                violations.append(
                    RuleViolation(self.id, self.violation_message.format(string), line)
                )
        ...
        for punctuation_mark in punctuation_marks:
            if title.endswith(punctuation_mark):
                return [
                    RuleViolation(
                        self.id,
                        f"Title has trailing punctuation ({punctuation_mark})",
                        title,
                    )
                ]
...
class TitleRegexMatches(LineRule):
    ...
    def validate(self, title, _commit):
        ...
        if not self.options["regex"].value.search(title):
            violation_msg = (
                f"Title does not match regex ({self.options['regex'].value.pattern})"
            )
...
class BodyFirstLineEmpty(CommitRule):
    ...
    def validate(self, commit):
        if len(commit.message.body) >= 1:
            ...
            if first_line != "":
                return [
                    RuleViolation(self.id, "Second line is not empty", first_line, 2)
                ]
...
class BodyMinLength(CommitRule):
    ...
    def validate(self, commit):
        min_length = self.options["min-length"].value
        body_message_no_newline = "".join(
            [line for line in commit.message.body if line is not None]
        )
        actual_length = len(body_message_no_newline)
        if 0 < actual_length < min_length:
            violation_message = (
                f"Body message is too short ({actual_length}<{min_length})"
            )
            return [
                RuleViolation(self.id, violation_message, body_message_no_newline, 3)
            ]
        ...
        for needs_mentioned_file in self.options["files"].value:
            ...
            if needs_mentioned_file in commit.changed_files:
                if needs_mentioned_file not in " ".join(commit.message.body):
                    violation_message = (
                        f"Body does not mention changed file '{needs_mentioned_file}'"
                    )
                    violations.append(
                        RuleViolation(
                            self.id,
...
class BodyRegexMatches(CommitRule):
    ...
    def validate(self, commit):
        ...
        if not self.options["regex"].value.search(full_body):
            violation_msg = (
                f"Body does not match regex ({self.options['regex'].value.pattern})"
            )
            return [
                RuleViolation(
                    self.id, violation_msg, None, len(commit.message.body) + 1
                )
            ]
...
class AuthorValidEmail(CommitRule):
    ...
    options_spec = [
        RegexOption(
            "regex",
            DEFAULT_AUTHOR_VALID_EMAIL_REGEX,
            "Regex that author email address should match",
        )
    ]
    ...
    def validate(self, commit):
        ...
        if commit.author_email and not regex_method(commit.author_email):
            return [
                RuleViolation(
                    self.id, "Author email for commit is invalid", commit.author_email
                )
            ]
    ...
    def apply(self, config, commit):
        ...
        for line in commit.message.body:
            if regex_method(line):
                ...
                self.log.debug(
                    "Ignoring commit because of rule '%s': %s", self.id, message
                )
...
class IgnoreBodyLines(ConfigurationRule):
    ...
    options_spec = [
        RegexOption(
            "regex", None, "Regex matching lines of the body that should be ignored"
        )
    ]
...
class IgnoreByAuthorName(ConfigurationRule):
    ...
    options_spec = [
        RegexOption(
            "regex",
            None,
            "Regex matching the author name of commits this rule should apply to",
        ),
        StrOption("ignore", "all", "Comma-separated list of rules to ignore"),
    ]
