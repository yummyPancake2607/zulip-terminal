    ...
    try:
        ...
        if "_cwd" in git_kwargs and b"not a git repository" in error_msg_lower:
            raise GitContextError(
                f"{git_kwargs['_cwd']} is not a git repository."
            ) from e
...
def git_commentchar(repository_path=None):
    ...
    commentchar = _git(
        "config", "--get", "core.commentchar", _cwd=repository_path, _ok_code=[0, 1]
    )
    # git will return an exit code of 1 if it can't find a config value, in this case we fall-back to # as commentchar
    if (
        hasattr(commentchar, "exit_code") and commentchar.exit_code == 1
...
class GitCommitMessage:
    ...
    def from_full_message(context, commit_msg_str):
        ...
        lines = [
            line
            for line in all_lines[:cutline_index]
            if not line.startswith(context.commentchar)
        ]
        full = "\n".join(lines)
        title = lines[0] if lines else ""
        body = lines[1:] if len(lines) > 1 else []
        return GitCommitMessage(
            context=context, original=commit_msg_str, full=full, title=title, body=body
        )
...
class GitChangedFileStats:
    ...
    def __repr__(self) -> str:
        return f'GitChangedFileStats(filepath="{self.filepath}", additions={self.additions}, deletions={self.deletions})'
    ...
    def __str__(self):
        ...
        if len(self.changed_files_stats) > 0:
            changed_files_stats_str = "\n  " + "\n  ".join(
                [str(stats) for stats in self.changed_files_stats.values()]
            )
        else:
            changed_files_stats_str = " {}"
...
class LocalGitCommit(GitCommit, PropertyCache):
    ...
    def _log(self):
        ...
        raw_commit = _git(
            "log", self.sha, "-1", long_format, _cwd=self.context.repository_path
        ).split("\n")
    ...
    def branches(self):
        def cache_branches():
            ...
            branches = _git(
                "branch", "--contains", self.sha, _cwd=self.context.repository_path
            ).split("\n")
            ...
            self._cache["branches"] = [
                branch.replace("*", "").strip() for branch in branches[:-1]
            ]
    ...
    def changed_files_stats(self):
        def cache_changed_files_stats():
            ...
            self._cache["changed_files_stats"] = _parse_git_changed_file_stats(
                changed_files_stats_raw
            )
...
class StagedLocalGitCommit(GitCommit, PropertyCache):
    ...
    def author_name(self):
        try:
            return _git(
                "config", "--get", "user.name", _cwd=self.context.repository_path
            ).strip()
        except GitExitCodeError as e:
            raise GitContextError(
                "Missing git configuration: please set user.name"
            ) from e
    ...
    def author_email(self):
        try:
            return _git(
                "config", "--get", "user.email", _cwd=self.context.repository_path
            ).strip()
        except GitExitCodeError as e:
            raise GitContextError(
    ...
    def from_local_repository(repository_path, refspec=None, commit_hashes=None):
        ...
        if refspec:
            ...
            for commit_hash in commit_hashes:
                sha_list.append(
                    _git(
                        "log", "-1", commit_hash, "--pretty=%H", _cwd=repository_path
                    ).replace("\n", "")
                )
            ...
            sha_list = [
                _git("log", "-1", "--pretty=%H", _cwd=repository_path).replace("\n", "")
            ]
