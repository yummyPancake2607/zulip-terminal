...
def log_system_info():
    ...
    LOG.debug(
        "GITLINT_USE_SH_LIB: %s", os.environ.get("GITLINT_USE_SH_LIB", "[NOT SET]")
    )
...
def build_config(  # pylint: disable=too-many-arguments
    ...
    if fail_without_commits:
        config_builder.set_option(
            "general", "fail-without-commits", fail_without_commits
        )
...
def build_git_context(lint_config, msg_filename, commit_hash, refspec):
    ...
    if lint_config.staged:
        ...
        from_commit_msg = lambda message: GitContext.from_staged_commit(  # pylint: disable=unnecessary-lambda-assignment
    ...
    if commit_hash and refspec:
        raise GitLintUsageError(
            "--commit and --commits are mutually exclusive, use one or the other."
        )
    ...
    if refspec:
        ...
        if "," in refspec:
            ...
            return GitContext.from_local_repository(
                lint_config.target, commit_hashes=commit_hashes
            )
        # 3.1.2 Real refspec
    ...
    if commit_hash:
        return GitContext.from_local_repository(
            lint_config.target, commit_hashes=[commit_hash]
        )
        ...
        try:
            ...
            if exit_code == GITLINT_SUCCESS:
                click.echo(
                    ...
                    + " (no violations in commit message)"
                )
                continue
            ...
            click.echo(
                "gitlint: "
                + click.style("Your commit message contains violations.", fg="red")
            )
            ...
            if value == "y":
                ...
                if msg_filename:
                    ...
                    click.echo(
                        "Editing only possible when --msg-filename is specified."
                    )
                    ctx.exit(exit_code)
            elif value == "n":
