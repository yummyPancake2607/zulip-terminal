...
class MatrixHttpApi(object):
    ...
    def create_room(
        self, alias=None, name=None, is_public=False, invitees=None, federate=None
    ):
        """Perform /createRoom.
    ...
    def get_state_event(self, room_id, event_type):
        ...
        """
        return self._send(
            "GET", "/rooms/{}/state/{}".format(quote(room_id), event_type)
        )
    ...
    def set_membership(
        self, room_id, user_id, membership, reason="", profile=None, timestamp=None
    ):
    ...
    def media_download(self, mxcurl, allow_remote=True):
        ...
        if mxcurl.startswith("mxc://"):
            ...
            raise ValueError("MXC URL '%s' did not begin with 'mxc://'" % mxcurl)
    ...
    def get_url_preview(self, url, ts=None):
        ...
        if ts:
            params["ts"] = ts
        return self._send(
            "GET", "", query_params=params, api_path="/_matrix/media/r0/preview_url"
        )
    ...
    def whoami(self):
        ...
        return self._send("GET", "/account/whoami")
