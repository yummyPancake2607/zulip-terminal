...
class MatrixClient(object):
    ...
    def __init__(
        ...
        if user_id:
            warn(
                "user_id is deprecated. " "Now it is requested from the server.",
                DeprecationWarning,
            )

        if encryption and not ENCRYPTION_SUPPORT:
            raise ValueError(
                "Failed to enable encryption. Please make sure the olm "
    ...
    def get_user(self, user_id):
        ...
        """
        warn(
            "get_user is deprecated. Directly instantiate a User instead.",
            DeprecationWarning,
        )
